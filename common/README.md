...
ResearcherのHuman Evidence integrity repairは`researcher.schemas.human_evidence`がcanonical ownerです。分類修復とReport完全一致deduplicationを`repair_kind` discriminatorで検証し、Researcherによる生成・保存からDeliberation、Conclusion、Playwright、Deliveryまで旧単一型を再定義しません。
