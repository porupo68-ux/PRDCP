...
class CompatibleClient:
    def inspect(self, model_id: str) -> ModelCapabilityResult:
        return ModelCapabilityResult(
            ...
            reason="CYCLE037_PRODUCTION_CLONE_MOCK_COMPATIBLE",
            endpoint_count=1,
...
def copy_workflow_boundary(source: Path, target: Path, workflow_id: str) -> None:
    ...
    boundaries = (
        ...
        ("provider_runtime_identity_repair_authorizations", "openrouter"),
...
async def verify(source: Path, workflow_id: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="prdcp_cycle037_clone_") as temporary:
        ...
        result = await manager.recover_runtime_provenance_contract(
            workflow_id,
        ...
        output = {
            ...
            "runtime_provenance_repair_authorizations": len(provenance_files),
