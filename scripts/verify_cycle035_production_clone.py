...
class CompatibleClient:
    def inspect(self, model_id: str) -> ModelCapabilityResult:
        return ModelCapabilityResult(
            ...
            reason="CYCLE035_PRODUCTION_CLONE_MOCK_COMPATIBLE",
            endpoint_count=1,
...
async def verify(source: Path, workflow_id: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="prdcp_cycle035_clone_") as temporary:
        ...
        result = await manager.recover_runtime_adapter_contract(
            workflow_id,
        ...
        output_files = list(
            (
                clone
                / "provider_runtime_adapter_repair_authorizations"
                / "openrouter"
                / workflow_id
            ).glob("*.json")
        )
        ...
        output = {
            ...
            "runtime_adapter_repair_authorizations": len(output_files),
