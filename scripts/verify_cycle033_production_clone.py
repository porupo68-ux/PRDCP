...
class CompatibleClient:
    def inspect(self, model_id: str) -> ModelCapabilityResult:
        return ModelCapabilityResult(
            ...
            reason="PRODUCTION_CLONE_MOCK_COMPATIBLE",
            endpoint_count=1,
...
def copy_workflow_boundary(source: Path, target: Path, workflow_id: str) -> None:
    ...
    reservation_source = source / "provider_call_reservations" / "openrouter" / workflow_id
    reservation_target = target / "provider_call_reservations" / "openrouter" / workflow_id
    shutil.copytree(reservation_source, reservation_target)
...
async def verify(source: Path, workflow_id: str) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="prdcp_cycle033_clone_") as temporary:
        ...
        # The clone keeps the production provider_id so it exercises the exact
        # old OpenRouter reservations.  Its deterministic test sources are still
        # generated by MockModelProvider, so suppress only the production-only
        # placeholder detector in this isolated clone.
        manager._is_placeholder_source = lambda _source: False
        audit = manager.inspect_retrieval_reconstruction(workflow_id)
        if audit["planned_retrieval_calls"] != 9:
            raise AssertionError(audit)
        result = await manager.reconstruct_missing_retrieval_and_recover(
            workflow_id,
            capability_client=CompatibleClient(),
        )
        if retrieval_provider.calls != 9:
                ...
                json.dumps(
                    {
                        ...
                        "tail_messages": [
                            ...
                            for message in latest.message_history[-4:]
                        ],
                    },
                    ensure_ascii=False,
                    indent=2,
        ...
        reconstruction_files = list(
            (
                clone
                / "retrieval_reconstruction_authorizations"
        ...
        output = {
            ...
            "reconstruction_authorizations": len(reconstruction_files),
            "runtime_repair_authorizations": len(runtime_files),
            "duplicate_calls": 0,
        }
    protected_after = {
        "state": sha256(source_state),
